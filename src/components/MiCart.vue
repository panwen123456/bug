  ...
  <div class="app-shell app-shell-bottom-navigation">
    ...
    <div class="app-view-wrapper">
      <div class="container fluid app-view app-view-with-header">
        <div class="page-wrap">
          <!-- 未登录时显示,点击后跳转到登录界面,登录后跳转到原来的路由,由于购物车不需要登录权限，因此全局设置的自动跳转目的 -->
            ...
            <ol>
              <li v-for="(item,index) in cartList" :key="item.goodsId" class="item">
                <div class="ui-flex align-center justify-center">
                  <div 
                  class="choose flex" :class="{checked:item.sel_status}"
                  @click="cartSelect(item,index)">
                    <!-- 如果不是商品的服务id则不勾选 -->
                    <i v-if="!item.parent_goodsId" class="iconfont"
                    :class="item.sel_status?'icon-roundcheckfill':'icon-round'"></i>
                  </div>
                  ...
                  <div class="info flex">
                    ...
                    <!-- 礼物没有价格 -->
                    <div v-if="!item.isGift" class="price-without">
                      <span>售价：</span>
                    ...
                    <div class="num">
                      <div class="xm-input-number">
                        <div 
                          :class="{active: item.price && item.num>1}"
                          class="input-sub"
                        ...
                        <div 
                          :class="{active: item.price && item.num<item.buy_limit}"
        ...
        <MiPop v-model="showServiceInfo" :overlay="true" 
        ...
        <!-- <div class="insurance-pop">
          <div v-show="showServiceInfo" class="ui-mask"></div>
          <div v-show="showServiceInfo" class="pop">
...
.noitems>a span {
  display: inline-block;
  line-height: 40px;
  background: url(../assets/imgs/cart_noitems.png) no-repeat 0;
